...
public class ArrayNode extends ContainerNode<ArrayNode> implements Serializable {
  ...
  public ArrayNode(JsonNodeFactory nf, List<JsonNode> children) {
    super(nf);
    this._children = Objects.<List<JsonNode>>requireNonNull(children, "Must not pass `null` for 'children' argument");
  }
  ...
  protected ObjectNode _withObjectAddTailElement(JsonPointer tail, boolean preferIndex) {
    ...
    if (preferIndex && tail.mayMatchElement()) {
      ...
      _withXxxSetArrayElement(index, arrayNode);
      return arrayNode._withObjectAddTailElement(tail, preferIndex);
    } 
  ...
  protected ArrayNode _withArrayAddTailElement(JsonPointer tail, boolean preferIndex) {
    ...
    if (preferIndex && tail.mayMatchElement()) {
      ArrayNode arrayNode = arrayNode();
      _withXxxSetArrayElement(index, arrayNode);
      return arrayNode._withArrayAddTailElement(tail, preferIndex);
    } 
    ObjectNode next = objectNode();
    _withXxxSetArrayElement(index, next);
  ...
  public boolean equals(Comparator<JsonNode> comparator, JsonNode o) {
    ...
    for (int i = 0; i < len; i++) {
      if (!((JsonNode)l1.get(i)).equals(comparator, l2.get(i)))
        return false; 
    } 
    return true;
  }
