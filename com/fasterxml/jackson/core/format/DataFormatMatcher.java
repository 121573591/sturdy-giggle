...
public class DataFormatMatcher {
  ...
  protected DataFormatMatcher(InputStream in, byte[] buffered, int bufferedStart, int bufferedLength, JsonFactory match, MatchStrength strength) {
    ...
    if ((bufferedStart | bufferedLength) < 0 || bufferedStart + bufferedLength > buffered.length)
      throw new IllegalArgumentException(String.format("Illegal start/length (%d/%d) wrt input array of %d bytes", new Object[] { Integer.valueOf(bufferedStart), Integer.valueOf(bufferedLength), Integer.valueOf(buffered.length) })); 
  }
  ...
  public InputStream getDataStream() {
    ...
    return (InputStream)new MergedStream(null, this._originalStream, this._bufferedData, this._bufferedStart, this._bufferedLength);
  }
}
