...
final class DescriptorMessageInfoFactory implements MessageInfoFactory {
  ...
  private static MessageInfo convert(Class<?> messageType, Descriptors.Descriptor messageDescriptor) {
    ...
    for (int i = 0; i < fieldDescriptors.size(); i++) {
      ...
      if (fd.getJavaType() == Descriptors.FieldDescriptor.JavaType.ENUM && fd
      ...
      if (fd.getRealContainingOneof() != null) {
        ...
        if (!fd.hasPresence()) {
          ...
          if (fd.isMapField()) {
            ...
            if (valueField.getJavaType() == Descriptors.FieldDescriptor.JavaType.ENUM && valueField
              .legacyEnumFieldTreatedAsClosed())
              enumVerifier = new Internal.EnumVerifier() {
                  public boolean isInRange(int number) {
            ...
            fieldImplicitPresence = FieldInfo.forMapField(field, number, 
            ...
            fieldImplicitPresence = FieldInfo.forRepeatedMessageField(field, number, type, 
