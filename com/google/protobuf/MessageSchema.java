...
final class MessageSchema<T> implements Schema<T> {
  ...
  static <T> MessageSchema<T> newSchemaForRawMessageInfo(RawMessageInfo messageInfo, NewInstanceSchema newInstanceSchema, ListFieldSchema listFieldSchema, UnknownFieldSchema<?, ?> unknownFieldSchema, ExtensionSchema<?> extensionSchema, MapFieldSchema mapFieldSchema) {
    ...
    while (i < length) {
      ...
      if (fieldType >= 51) {
        ...
        if (oneofFieldType == 9 || oneofFieldType == 17) {
          ...
          if (messageInfo.getSyntax().equals(ProtoSyntax.PROTO2) || (fieldTypeWithExtraBits & 0x800) != 0)
            objects[bufferIndex / 3 * 2 + 1] = messageInfoObjects[objectsPosition++]; 
        } 
        ...
        boolean hasHasBit = ((fieldTypeWithExtraBits & 0x1000) != 0);
      ...
      buffer[bufferIndex++] = (
        ((fieldTypeWithExtraBits & 0x200) != 0) ? true : false) | (
        ((fieldTypeWithExtraBits & 0x100) != 0) ? true : false) | (
        ((fieldTypeWithExtraBits & 0x800) != 0) ? Integer
        .MIN_VALUE : 
        0) | fieldType << 20 | fieldOffset;
    ...
    return new MessageSchema<>(buffer, objects, minFieldNumber, maxFieldNumber, messageInfo
        ...
        .getDefaultInstance(), messageInfo
        .getSyntax(), false, intArray, checkInitialized, checkInitialized + mapFieldCount, newInstanceSchema, listFieldSchema, unknownFieldSchema, extensionSchema, mapFieldSchema);
  }
  ...
  static <T> MessageSchema<T> newSchemaForMessageInfo(StructuralMessageInfo messageInfo, NewInstanceSchema newInstanceSchema, ListFieldSchema listFieldSchema, UnknownFieldSchema<?, ?> unknownFieldSchema, ExtensionSchema<?> extensionSchema, MapFieldSchema mapFieldSchema) {
    ...
    return new MessageSchema<>(buffer, objects, minFieldNumber, maxFieldNumber, messageInfo
        ...
        .getDefaultInstance(), messageInfo
        .getSyntax(), true, combined, checkInitialized.length, checkInitialized.length + mapFieldPositions.length, newInstanceSchema, listFieldSchema, unknownFieldSchema, extensionSchema, mapFieldSchema);
  }
  ...
  public int getSerializedSize(T message) {
    ...
    for (int i = 0; i < this.buffer.length; i += 3) {
      ...
      switch (fieldType) {
        case 0:
          if (isFieldPresent(message, i, currentPresenceFieldOffset, currentPresenceField, presenceMask))
          ...
          switch (fieldType) {
            ...
            case 12:
              if (wireType == 0) {
                ...
                if (!isLegacyEnumIsClosed(typeAndOffset) || enumVerifier == null || enumVerifier
