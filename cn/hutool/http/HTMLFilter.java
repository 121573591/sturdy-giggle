...
public final class HTMLFilter {
  ...
  private static final ConcurrentMap<String, Pattern> P_REMOVE_PAIR_BLANKS = (ConcurrentMap<String, Pattern>)new SafeConcurrentHashMap();
  
  private static final ConcurrentMap<String, Pattern> P_REMOVE_SELF_BLANKS = (ConcurrentMap<String, Pattern>)new SafeConcurrentHashMap();
  ...
  private String decodeEntities(String s) {
    ...
    while (m.find()) {
      String match = m.group(1);
      int decimal = Integer.parseInt(match, 16);
      m.appendReplacement(buf, Matcher.quoteReplacement(chr(decimal)));
    } 
    m.appendTail(buf);
    s = buf.toString();
