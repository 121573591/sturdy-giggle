...
public class CpuInfo {
  ...
  public String toString() {
    return "CpuInfo{CPU核心数=" + this.cpuNum + ", CPU总的使用率=" + this.toTal + ", CPU系统使用率=" + this.sys + ", CPU用户使用率=" + this.user + ", CPU当前等待率=" + this.wait + ", CPU当前空闲率=" + this.free + ", CPU利用率=" + 
      
      getUsed() + ", CPU型号信息='" + this.cpuModel + '\'' + '}';
  }
  ...
  private static double formatDouble(long tick, long totalCpu) {
    if (0L == totalCpu)
      return 0.0D; 
    return Double.parseDouble(LOAD_FORMAT.format((tick <= 0L) ? 0.0D : (100.0D * tick / totalCpu)));
  }
}
