...
public class PatternParser {
  ...
  private static PatternMatcher parseSingle(String pattern) {
    ...
    Assert.checkBetween(parts.length, 5, 7, () -> new CronException("Pattern [{}] is invalid, it must be 5-7 parts!", new Object[] { pattern }));
    int offset = 0;
    if (parts.length == 6 || parts.length == 7)
      offset = 1; 
    String secondPart = (1 == offset) ? parts[0] : "0";
    if (parts.length == 7) {
      PartMatcher yearMatcher = YEAR_VALUE_PARSER.parse(parts[6]);
    } else {
      alwaysTrueMatcher = AlwaysTrueMatcher.INSTANCE;
    } 
    return new PatternMatcher(SECOND_VALUE_PARSER
        
        .parse(secondPart), MINUTE_VALUE_PARSER
        
        .parse(parts[offset]), HOUR_VALUE_PARSER
        
        .parse(parts[1 + offset]), DAY_OF_MONTH_VALUE_PARSER
        
        .parse(parts[2 + offset]), MONTH_VALUE_PARSER
        
        .parse(parts[3 + offset]), DAY_OF_WEEK_VALUE_PARSER
        
        .parse(parts[4 + offset]), (PartMatcher)alwaysTrueMatcher);
  }
}
