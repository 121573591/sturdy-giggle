...
public class PatternMatcher {
  ...
  private boolean match(int second, int minute, int hour, int dayOfMonth, int month, int dayOfWeek, int year) {
    return ((second < 0 || this.matchers[0].match(Integer.valueOf(second))) && this.matchers[1]
      .match(Integer.valueOf(minute)) && this.matchers[2]
      .match(Integer.valueOf(hour)) && 
      matchDayOfMonth(this.matchers[3], dayOfMonth, month, Year.isLeap(year)) && this.matchers[4]
      .match(Integer.valueOf(month)) && this.matchers[5]
      .match(Integer.valueOf(dayOfWeek)) && this.matchers[6]
      .match(Integer.valueOf(year)));
  }
  
  private static boolean matchDayOfMonth(PartMatcher matcher, int dayOfMonth, int month, boolean isLeapYear) {
    return (matcher instanceof DayOfMonthMatcher) ? ((DayOfMonthMatcher)matcher)
      .match(dayOfMonth, month, isLeapYear) : matcher
      .match(Integer.valueOf(dayOfMonth));
  }
