...
public class MailAccount implements Serializable {
  ...
  public Properties getSmtpProps() {
    ...
    if (this.starttlsEnable) {
      ...
      if (null == this.sslEnable)
        this.sslEnable = Boolean.valueOf(true); 
    } 
  ...
  public MailAccount defaultIfEmpty() {
    ...
    if (StrUtil.isBlank(this.host))
      this.host = StrUtil.format("smtp.{}", new Object[] { StrUtil.subSuf(fromAddress, fromAddress.indexOf('@') + 1) }); 
    if (StrUtil.isBlank(this.user))
      this.user = fromAddress; 
    if (null == this.auth)
      this.auth = Boolean.valueOf((false == StrUtil.isBlank(this.pass))); 
    if (null == this.port)
      this.port = Integer.valueOf((null != this.sslEnable && this.sslEnable.booleanValue()) ? this.socketFactoryPort : 25); 
