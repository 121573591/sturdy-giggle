...
public class StrSplitter {
  ...
  public static <R> List<R> split(CharSequence text, char separator, int limit, boolean ignoreEmpty, boolean ignoreCase, Function<String, R> mapping) {
    ...
    SplitIter splitIter = new SplitIter(text, (TextFinder)new CharFinder(separator, ignoreCase), limit, ignoreEmpty);
  ...
  public static List<String> split(CharSequence text, String separator, int limit, boolean isTrim, boolean ignoreEmpty, boolean ignoreCase) {
    if (null == text)
      return new ArrayList<>(0); 
    SplitIter splitIter = new SplitIter(text, (TextFinder)new StrFinder(separator, ignoreCase), limit, ignoreEmpty);
  ...
  public static List<String> split(CharSequence text, int limit) {
    if (null == text)
      return new ArrayList<>(0); 
    SplitIter splitIter = new SplitIter(text, (TextFinder)new CharMatcherFinder(CharUtil::isBlankChar), limit, true);
  ...
  public static List<String> split(String text, Pattern separatorPattern, int limit, boolean isTrim, boolean ignoreEmpty) {
    if (null == text)
      return new ArrayList<>(0); 
    SplitIter splitIter = new SplitIter(text, (TextFinder)new PatternFinder(separatorPattern), limit, ignoreEmpty);
    return splitIter.toList(isTrim);
  }
  ...
  public static String[] splitByLength(CharSequence text, int len) {
    if (null == text)
      return new String[0]; 
    SplitIter splitIter = new SplitIter(text, (TextFinder)new LengthFinder(len), -1, false);
