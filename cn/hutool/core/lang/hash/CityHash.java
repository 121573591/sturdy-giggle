...
public class CityHash {
  ...
  public static int hash32(byte[] data) {
    ...
    if (len <= 24)
      return (len <= 12) ? ((len <= 4) ? 
        hash32Len0to4(data) : hash32Len5to12(data)) : 
        hash32Len13to24(data); 
    int h = len, g = -862048943 * len, f = g;
  ...
  public static long hash64(byte[] data) {
    ...
    while (true) {
      ...
      len -= 64;
      if (len == 0)
        return hashLen16(hashLen16(v.getLowValue(), w.getLowValue()) + shiftMix(y) * -5435081209227447693L + z, 
  ...
  public static Number128 hash128(byte[] data) {
    int len = data.length;
    return (len >= 16) ? 
      hash128(data, 16, new Number128(
          fetch64(data, 0), fetch64(data, 8) + -4348849565147123417L)) : 
      hash128(data, 0, new Number128(-4348849565147123417L, -5435081209227447693L));
  }
  ...
  private static Number128 hash128(byte[] byteArray, int start, Number128 seed) {
    ...
    long z = len * -5435081209227447693L;
    v.setLowValue(rotate64(y ^ 0xB492B66FBE98F273L, 49) * -5435081209227447693L + fetch64(byteArray, start));
    ...
    while (true) {
      ...
      len -= 128;
      if (len < 128) {
        x += rotate64(v.getLowValue() + z, 49) * -4348849565147123417L;
        y = y * -4348849565147123417L + rotate64(w.getHighValue(), 37);
        z = z * -4348849565147123417L + rotate64(w.getLowValue(), 27);
  ...
  private static long hashLen0to16(byte[] byteArray) {
    int len = byteArray.length;
    if (len >= 8) {
      long mul = -7286425919675154353L + len * 2L;
    ...
    if (len >= 4) {
      long mul = -7286425919675154353L + (len * 2);
      long a = fetch32(byteArray, 0) & 0xFFFFFFFFL;
      return hashLen16(len + (a << 3L), fetch32(byteArray, len - 4) & 0xFFFFFFFFL, mul);
    } 
    if (len > 0) {
  ...
  private static long hashLen17to32(byte[] byteArray) {
    ...
    return hashLen16(rotate64(a + b, 43) + rotate64(c, 30) + d, a + 
        rotate64(b + -7286425919675154353L, 18) + c, mul);
  }
  ...
  private static Number128 weakHashLen32WithSeeds(byte[] byteArray, int start, long a, long b) {
    return weakHashLen32WithSeeds(fetch64(byteArray, start), 
        ...
        fetch64(byteArray, start + 24), a, b);
  }
