...
public class BeanPath implements Serializable {
  ...
  private static Object getFieldValue(Object bean, String expression) {
    ...
    if (StrUtil.contains(expression, ':')) {
      List<String> parts = StrUtil.splitTrim(expression, ':');
      ...
      if (StrUtil.contains(expression, ',')) {
        List<String> keys = StrUtil.splitTrim(expression, ',');
        if (bean instanceof Collection)
          return CollUtil.getAny((Collection)bean, (int[])Convert.convert(int[].class, keys)); 
        if (ArrayUtil.isArray(bean))
          return ArrayUtil.getAny(bean, (int[])Convert.convert(int[].class, keys)); 
        ...
        if (bean instanceof Map)
          return MapUtil.getAny((Map)bean, (Object[])unWrappedKeys); 
        Map<String, Object> map = BeanUtil.beanToMap(bean, new String[0]);
        return MapUtil.getAny(map, (Object[])unWrappedKeys);
      } 
  ...
  private void init(String expression) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (0 == i && '$' == c) {
        ...
        if (']' == c) {
          if (false == isNumStart)
            throw new IllegalArgumentException(StrUtil.format("Bad expression '{}':{}, we find ']' but no '[' !", new Object[] { expression, Integer.valueOf(i) })); 
          isNumStart = false;
        } else {
          if (isNumStart)
            throw new IllegalArgumentException(StrUtil.format("Bad expression '{}':{}, we find '[' but no ']' !", new Object[] { expression, Integer.valueOf(i) })); 
    ...
    if (isNumStart)
      throw new IllegalArgumentException(StrUtil.format("Bad expression '{}':{}, we find '[' but no ']' !", new Object[] { expression, Integer.valueOf(length - 1) })); 
    if (builder.length() > 0)
      localPatternParts.add(builder.toString()); 
