...
public class CellUtil {
  ...
  public static Object getCellValue(Cell cell, CellType cellType, boolean isTrimCellValue) {
    return getCellValue(cell, cellType, isTrimCellValue ? (CellEditor)new TrimEditor() : null);
  }
  ...
  public static boolean isMergedRegion(Sheet sheet, int x, int y) {
    if (sheet != null) {
      ...
      for (int i = 0; i < sheetMergeCount; i++) {
        ...
        if (y >= ca.getFirstRow() && y <= ca.getLastRow() && x >= ca
          .getFirstColumn() && x <= ca.getLastColumn())
          return true; 
      } 
    } 
    return false;
  ...
  public static Cell getMergedRegionCell(Cell cell) {
    if (null == cell)
      return null; 
    return (Cell)ObjectUtil.defaultIfNull(
        getCellIfMergedRegion(cell.getSheet(), cell.getColumnIndex(), cell.getRowIndex()), cell);
  }
  
  public static Cell getMergedRegionCell(Sheet sheet, int x, int y) {
    return (Cell)ObjectUtil.defaultIfNull(
        getCellIfMergedRegion(sheet, x, y), () -> SheetUtil.getCell(sheet, y, x));
  }
