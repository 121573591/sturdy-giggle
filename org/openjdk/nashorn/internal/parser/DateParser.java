...
public class DateParser {
      ...
      switch (this.token) {
        case NUMBER:
          ...
          if (this.currentField < 7)
            set(this.currentField++, this.numValue); 
          break;
        case NAME:
          ...
          if (skipDelimiter(':')) {
            ...
            do {
              ...
              if (this.token != Token.NUMBER || !setTimeField(this.numValue))
                return false; 
            } while (skipDelimiter(isSet(5) ? 46 : 58));
            break;
          } 
  ...
  private boolean skipNumberDelimiter(char c) {
    if (this.pos < this.length - 1 && this.string.charAt(this.pos) == c && 
      Character.getType(this.string.charAt(this.pos + 1)) == 9) {
      this.token = null;
      this.pos++;
      return true;
    } 
  ...
  private boolean patchResult(boolean strict) {
    ...
    } else if (get(3).intValue() == 24 && (
      get(4).intValue() != 0 || get(5).intValue() != 0 || get(6).intValue() != 0)) {
      return false;
    } 
