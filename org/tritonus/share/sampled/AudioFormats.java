...
public class AudioFormats {
  ...
  public static boolean matches(AudioFormat format1, AudioFormat format2) {
    return (format1.getEncoding().equals(format2.getEncoding()) && (format2
      .getSampleSizeInBits() <= 8 || format1
      .getSampleSizeInBits() == -1 || format2
      .getSampleSizeInBits() == -1 || format1
      .isBigEndian() == format2.isBigEndian()) && 
  ...
  public static boolean equals(AudioFormat format1, AudioFormat format2) {
    return (format1.getEncoding().equals(format2.getEncoding()) && format1
      .getChannels() == format2.getChannels() && format1
      .getSampleSizeInBits() == format2.getSampleSizeInBits() && format1
      .getFrameSize() == format2.getFrameSize() && 
      Math.abs(format1.getSampleRate() - format2.getSampleRate()) < 1.0E-9D && 
      Math.abs(format1.getFrameRate() - format2.getFrameRate()) < 1.0E-9D);
  }
}
