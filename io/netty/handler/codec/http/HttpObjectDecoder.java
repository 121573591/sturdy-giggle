    ...
    switch (this.currentState) {
      ...
      case READ_INITIAL:
        try {
          ...
          out.add(invalidMessage(this.message, buffer, e));
          return;
        } 
      case READ_HEADER:
        try {
          ...
          if (this.contentLength == 0L || (this.contentLength == -1L && isDecodingRequest())) {
            addCurrentMessage(out);
            out.add(LastHttpContent.EMPTY_LAST_CONTENT);
            resetNow();
            return;
  ...
  protected void decodeLast(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
    ...
    switch (this.currentState) {
      case READ_VARIABLE_LENGTH_CONTENT:
        ...
        return;
      case READ_HEADER:
        out.add(invalidMessage(this.message, Unpooled.EMPTY_BUFFER, (Exception)new PrematureChannelClosureException("Connection closed before received headers")));
        resetNow();
        return;
      case READ_CHUNK_SIZE:
      case READ_FIXED_LENGTH_CONTENT:
