...
public class RtspDecoder extends HttpObjectDecoder {
  ...
  protected HttpMessage createMessage(String[] initialLine) throws Exception {
    if (versionPattern.matcher(initialLine[0]).matches()) {
      ...
      return (HttpMessage)new DefaultHttpResponse(RtspVersions.valueOf(initialLine[0]), new HttpResponseStatus(
            Integer.parseInt(initialLine[1]), initialLine[2]), this.headersFactory);
    } 
    this.isDecodingRequest = true;
    return (HttpMessage)new DefaultHttpRequest(RtspVersions.valueOf(initialLine[2]), 
        RtspMethods.valueOf(initialLine[0]), initialLine[1], this.headersFactory);
  }
  ...
  protected HttpMessage createInvalidMessage() {
    if (this.isDecodingRequest)
      return (HttpMessage)new DefaultFullHttpRequest(RtspVersions.RTSP_1_0, RtspMethods.OPTIONS, "/bad-request", 
          Unpooled.buffer(0), this.headersFactory, this.trailersFactory); 
    return (HttpMessage)new DefaultFullHttpResponse(RtspVersions.RTSP_1_0, UNKNOWN_STATUS, 
        Unpooled.buffer(0), this.headersFactory, this.trailersFactory);
  }
