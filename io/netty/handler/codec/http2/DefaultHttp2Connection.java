...
public class DefaultHttp2Connection implements Http2Connection {
  ...
  public Future<Void> close(Promise<Void> promise) {
    ...
    if (this.closePromise != null) {
      if (this.closePromise != promise)
        if (promise instanceof io.netty.channel.ChannelPromise && ((ChannelFuture)this.closePromise).isVoid()) {
          this.closePromise = promise;
        } else {
          PromiseNotifier.cascade((Future)this.closePromise, promise);
        }  
    } else {
      this.closePromise = promise;
    } 
  ...
  public void goAwayReceived(int lastKnownStream, long errorCode, ByteBuf debugData) throws Http2Exception {
    if (this.localEndpoint.lastStreamKnownByPeer() >= 0 && this.localEndpoint.lastStreamKnownByPeer() < lastKnownStream)
      throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, "lastStreamId MUST NOT increase. Current value: %d new value: %d", new Object[] { Integer.valueOf(this.localEndpoint.lastStreamKnownByPeer()), Integer.valueOf(lastKnownStream) }); 
  ...
  public boolean goAwaySent(int lastKnownStream, long errorCode, ByteBuf debugData) throws Http2Exception {
    if (this.remoteEndpoint.lastStreamKnownByPeer() >= 0) {
      if (lastKnownStream == this.remoteEndpoint.lastStreamKnownByPeer())
        return false; 
      if (lastKnownStream > this.remoteEndpoint.lastStreamKnownByPeer())
        throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, "Last stream identifier must not increase between sending multiple GOAWAY frames (was '%d', is '%d').", new Object[] { Integer.valueOf(this.remoteEndpoint.lastStreamKnownByPeer()), Integer.valueOf(lastKnownStream) }); 
    } 
  ...
  private class DefaultStream implements Http2Stream {
    ...
    public Http2Stream open(boolean halfClosed) throws Http2Exception {
      ...
      DefaultHttp2Connection.DefaultEndpoint<? extends Http2FlowController> endpoint = createdBy();
      if (!endpoint.canOpenStream())
        throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, "Maximum active streams violated for this endpoint: " + endpoint
            .maxActiveStreams(), new Object[0]); 
      activate();
      return this;
    }
  ...
  private final class DefaultEndpoint<F extends Http2FlowController> implements Http2Connection.Endpoint<F> {
    ...
    DefaultEndpoint(boolean server, int maxReservedStreams) {
      this.lastCreatedStreamIdentity = 0L;
      this.server = server;
      if (server) {
    ...
    public DefaultHttp2Connection.DefaultStream createStream(int streamId, boolean halfClosed) throws Http2Exception {
      ...
      DefaultHttp2Connection.DefaultStream stream = new DefaultHttp2Connection.DefaultStream(this.lastCreatedStreamIdentity, streamId, state);
    ...
    private void checkNewStreamAllowed(int streamId, Http2Stream.State state) throws Http2Exception {
      ...
      if (this.lastStreamKnownByPeer >= 0 && streamId > this.lastStreamKnownByPeer)
        throw Http2Exception.streamError(streamId, Http2Error.REFUSED_STREAM, "Cannot create stream %d greater than Last-Stream-ID %d from GOAWAY.", new Object[] { Integer.valueOf(streamId), Integer.valueOf(this.lastStreamKnownByPeer) }); 
