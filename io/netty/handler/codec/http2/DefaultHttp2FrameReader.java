...
public class DefaultHttp2FrameReader implements Http2FrameReader, Http2FrameSizePolicy, Http2FrameReader.Configuration {
    ...
    if (this.flags.ack()) {
      ...
      for (int index = 0; index < numSettings; index++) {
        ...
        try {
          ...
          if (id == '\004')
            throw Http2Exception.connectionError(Http2Error.FLOW_CONTROL_ERROR, e, "Failed setting initial window size: %s", new Object[] { e
                  .getMessage() }); 
          throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, e, "Protocol error: %s", new Object[] { e.getMessage() });
        } 
      } 
  ...
  private void readContinuationFrame(ByteBuf payload, int payloadEndIndex, Http2FrameListener listener) throws Http2Exception {
    this.headersContinuation.processFragment(this.flags.endOfHeaders(), payload, payloadEndIndex - payload
  ...
  private abstract class HeadersContinuation {
    ...
    abstract void processFragment(boolean param1Boolean, ByteBuf param1ByteBuf, int param1Int, Http2FrameListener param1Http2FrameListener) throws Http2Exception;
  }
  ...
  protected class HeadersBlockBuilder {
    ...
    final void addFragment(ByteBuf fragment, int len, ByteBufAllocator alloc, boolean endOfHeaders) throws Http2Exception {
      ...
      if (DefaultHttp2FrameReader.this.headersDecoder.configuration().maxHeaderListSizeGoAway() - len < this.headerBlock
        .readableBytes())
        headerSizeExceeded(); 
