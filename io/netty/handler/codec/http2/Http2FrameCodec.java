...
public class Http2FrameCodec extends Http2ConnectionHandler {
  ...
  private final IntObjectMap<DefaultHttp2FrameStream> frameStreamToInitializeMap = (IntObjectMap<DefaultHttp2FrameStream>)new IntObjectHashMap(8);
  ...
  private void writeHeadersFrame(ChannelHandlerContext ctx, Http2HeadersFrame headersFrame, ChannelPromise promise) {
    if (Http2CodecUtil.isStreamIdValid(headersFrame.stream().id())) {
      ...
      if (!promise.isDone()) {
        ...
        handleHeaderFuture((ChannelFuture)promise, streamId);
      } 
    } 
  }
  ...
  private final class ConnectionListener extends Http2ConnectionAdapter {
    ...
    public void onStreamAdded(Http2Stream stream) {
      Http2FrameCodec.DefaultHttp2FrameStream frameStream = (Http2FrameCodec.DefaultHttp2FrameStream)Http2FrameCodec.this.frameStreamToInitializeMap.remove(stream.id());
      if (frameStream != null)
  ...
  private final class FrameListener implements Http2FrameListener {
    ...
    public void onPriorityRead(ChannelHandlerContext ctx, int streamId, int streamDependency, short weight, boolean exclusive) {
      ...
      if (stream == null)
        return; 
      Http2FrameCodec.this.onHttp2Frame(ctx, (new DefaultHttp2PriorityFrame(streamDependency, weight, exclusive))
          .stream(requireStream(streamId)));
    }
    ...
    public void onPushPromiseRead(ChannelHandlerContext ctx, int streamId, int promisedStreamId, Http2Headers headers, int padding) {
      Http2FrameCodec.this.onHttp2Frame(ctx, (new DefaultHttp2PushPromiseFrame(headers, padding, promisedStreamId))
  ...
  private final class Http2RemoteFlowControllerListener implements Http2RemoteFlowController.Listener {
    ...
    public void writabilityChanged(Http2Stream stream) {
      Http2FrameCodec.DefaultHttp2FrameStream frameStream = stream.<Http2FrameCodec.DefaultHttp2FrameStream>getProperty(Http2FrameCodec.this.streamKey);
      if (frameStream == null)
        return; 
      Http2FrameCodec.this.onHttp2StreamWritabilityChanged(Http2FrameCodec.this.ctx, frameStream, ((Http2RemoteFlowController)Http2FrameCodec.this
          .connection().remote().flowController()).isWritable(stream));
    }
  }
