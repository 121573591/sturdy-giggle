    ...
    switch (key) {
      ...
      case 4:
        if (value.longValue() < 0L || value.longValue() > 2147483647L)
          throw new IllegalArgumentException("Setting INITIAL_WINDOW_SIZE is invalid: " + value + ", expected [" + Character.MIN_VALUE + ", " + 2147483647 + ']'); 
        return;
      case 5:
        if (!Http2CodecUtil.isMaxFrameSizeValid(value.intValue()))
          throw new IllegalArgumentException("Setting MAX_FRAME_SIZE is invalid: " + value + ", expected [" + '䀀' + ", " + 16777215 + ']'); 
        return;
      case 6:
        if (value.longValue() < 0L || value.longValue() > 4294967295L)
